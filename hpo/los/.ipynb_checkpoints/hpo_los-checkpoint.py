...
if args.model == "gbt":
    ...
    model_grid.fit(train_X, train_y, val_X, val_y, scoring="accuracy")
    print(model_grid.get_best_params())
    
    with open(f"best_configs/{args.model}_los_params.yaml", 'w') as f:
    ...
    tuner = keras_tuner.RandomSearch(
        ...
        project_name=f"{args.model}_los_hpo_checkpoints",
    )

    callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_acc', mode="max", min_delta=1e-4, patience=5)]
    tuner.search(
        train_dataset,
        epochs=100,
